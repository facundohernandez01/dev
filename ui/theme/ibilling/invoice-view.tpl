        ...
        <div class="col-lg-12"  id="application_ajaxrender">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <div class="btn-group  pull-right" role="group" aria-label="...">
                        ...
                        {if $_c['accounting'] eq '1'}
                            {*<button type="button" class="btn  btn btn-sm" id="add_payment"><i class="fa fa-plus"></i> {$_L['Add Payment']}</button>*}
                            <div class="btn-group" role="group">
                            <button type="button" class="btn  btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            ...
                            {if $d['remito'] neq 1}
                            <ul class="dropdown-menu" role="menu">
                            <li><button type="button" class="btn  btn-danger btn-sm" id="convert_aremito"><i class="fa fa-plus"></i> {$_L['Convert to remito']}</button></li>
                            {if $d['remito'] neq 3}
                ...
                <div class="ibox-content">
                        ...
                        <header class="clearfix">
                            <div class="row">
                                <div class="col-sm-6 mt-md">
                                    <h2 class="h2 mt-none mb-sm text-dark text-bold">{$_L['INVOICE']} #{$d['invoicenum']}{if $d['cn'] neq ''} {$d['cn']} {else} {$d['id']} {/if}</h2>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="bill-to">
                                        ...
                                        <address>
                                            {if $a['company'] neq ''}
                                                <strong>Cliente: </strong>{$a['company']}
                                                <br>
                                                <strong>{$_L['ATTN']}:</strong>  {$a['account']}
                                                <br>
                                            {else}
                                                {$d['account']}
                                                <br>
                                            ...
                                            <strong>Localización: </strong>{$a['address']}, {$a['city']}, {$a['state']} - {$a['zip']}, {$a['country']}
                                            <br>
                    ...
                    {if $d['remito'] neq '0'}                     
                       <h3>Historial pedido</h3>
                        <div class="table-responsive">
                                ...
                                <tbody>
                            {foreach $pedidos_remito_sub as $p_rsub}
                            <tr>
                            <td>{$p_rsub['description']}</td>
                            <td class="text-center">{$p_rsub['qty']}</td> 
                            <td class="text-center">{$p_rsub['duedate']}</td> 
                            <td class="text-center"><a href="{$_url}remitos/view/{$p_rsub['invoiceid']}">0001000{$p_rsub['invoiceid']}</a></td>
                    ...
                    {if ($pedidos_recibos neq '' || $pedidos_recibos neq '0')}
                     <table class="table table-bordered sys_table">
          
                            {foreach $pedidos_recibos as $p_rec}
                            <tr>
                            <div class="alert alert-danger" role="alert">
                            Recibo: <a href="{$_url}recibos/view/{$p_rec['id']}">{$p_rec['invoicenum']}{$p_rec['id']}</a>
                            Fecha: {date( $_c['df'], strtotime($p_rec['date']))}
                            Total: {$p_rec['total']}
                            </div>
                            </tr>

                            {foreach $pedidos_recibos_sub as $p_recsub}
                            <tr>
                            <td>-> Item: {$p_recsub['description']}</td>
                            <td>Cant: {$p_recsub['qty']}</td>
                            <td>Importe: {$p_recsub['total']}</td>
                            </tr>
