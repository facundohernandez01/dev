...
/* EJEMPLO ONFOCUS
    ...
    */
    ...
    $modal.on('click', '#add_item', function(){
        $modal.modal('loading');
            var _url = $("#_url").val();
            $.post(_url + 'ps/add-post/', $('#edit_form').serialize(), function(data){
                $modal.modal('hide');
    ...
    $('#company_add').on('click', function(e){
        ...
        $modal.load( _url + 'quotes/modal_add_company_from/', '', function(){
            $modal.modal();
            $("#ajax-modal .country").select2({
                theme: "bootstrap"
            });
        ...
        $.post( _url + "quotes/add_company_post_from/", $("#ib_modal_form").serialize())
            .done(function( data ) {

                if ($.isNumeric(data)) {

                    window.location = base_url + 'quotes/new/' +  '/' + data;
    ...
    var $modal = $('#ajax-modal');
    var sysrender = $('#application_ajaxrender');
    sysrender.on('click', '.cnew', function(e){
        e.preventDefault();
        var vid = this.id;
        var id = vid.replace("e", "");
        var id = id.replace("t", "");
       ...
       // var id = $(this).closest('tr').attr('id');
        ...
        var _url = $("#_url").val();
        setTimeout(function(){
            $modal.load(_url + 'quotes/add-form/', function(){
                $modal.modal();
