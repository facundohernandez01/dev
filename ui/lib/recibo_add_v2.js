$(document).ready(function () {
    ...
    $recibo_items.on('change', 'select', function(){
        //   $('#taxtotal').html('dd');
    ...
    item_remove.on('click', function(){
        $("#recibo_items tr.info").fadeOut(300, function(){
            $(this).remove();
    ...
    $('#company_add').on('click', function(e){
        ...
        $.post( _url + "recibos/add_company_post_from/", $("#ib_modal_form").serialize())
            .done(function( data ) {

                if ($.isNumeric(data)) {

                    window.location = base_url + 'recibos/add/' +  '/' + data;
    ...
    $recibo_items.on('click', '.redactor-editor', function(){
        $("tr").removeClass("info");
    ...
    $("#submit").click(function (e) {
        ...
        var _url = $("#_url").val();
        $.post(_url + 'recibos/add-post/', $('#invform').serialize(), function(data){

            var _url = $("#_url").val();
            if ($.isNumeric(data)) {

                window.location = _url + 'recibos/edit/' + data + '/';
            }
            else {
                $('#ibox_form').unblock();
                var body = $("html, body");
    ...
    $("#save_n_close").click(function (e) {
        ...
        $.post(_url + 'recibos/add-post/', $('#invform').serialize(), function(data){
            ...
            if ($.isNumeric(data)) {
                ...
                window.location = _url + 'recibos/view/' + data + '/';
            }
            else {
                $('#ibox_form').unblock();
                var body = $("html, body");
