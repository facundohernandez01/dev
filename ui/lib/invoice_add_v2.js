$(document).ready(function () {
    ...
    $('#company_add').on('click', function(e){
        ...
        $modal.load( _url + 'invoices/modal_add_company_from/', '', function(){
            $modal.modal();
            $("#ajax-modal .country").select2({
                theme: "bootstrap"
            });
        ...
        $.post( _url + "invoices/add_company_post_from/", $("#ib_modal_form").serialize())
            .done(function( data ) {

                if ($.isNumeric(data)) {

                    window.location = base_url + 'invoices/add/' +  '/' + data;
