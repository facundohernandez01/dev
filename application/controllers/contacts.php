...
_L[\'New Company\'] = \'' .
 ...
 '
	...
	case 'recibos':
        Event::trigger('contacts/recibos/');
        $cid = _post('cid');
        $ui->assign('cid', $cid);
        $d = ORM::for_table('crm_accounts')->find_one($cid);
        if ($d) {
            $i = ORM::for_table('sys_recibos')
                ->where('userid', $cid)
                ->find_many();
            $ui->assign('i', $i);
            $ui->display('ajax.contact-recibos.tpl');
        } else {
        }
        break;

    case 'remitos':
            Event::trigger('contacts/remitos/');
            $cid = _post('cid');
            $ui->assign('cid', $cid);
            $d = ORM::for_table('crm_accounts')->find_one($cid);
            if ($d) {
                $i = ORM::for_table('sys_remitos')
                    ->where('userid', $cid)                   
                    ->find_many();
                $ui->assign('i', $i);
                $ui->display('ajax.contact-remitos.tpl');
            } else {
            }
    break;
    ...
    case 'quotes':
        ...
        $ui->assign('cid', $cid);
        $d = ORM::for_table('crm_accounts')->find_one($cid);
        $com = ORM::for_table('sys_companies')
        ->where('id', $d['cid'])
        ->select('id')
        ->find_many();
        $ui->assign('d', $d);
    ...
    case 'view':
        ...
        if ($d) {
            ...
            $recibos_count = ORM::for_table('sys_recibos')
                ->where('userid', $id)
                ->count();
            ...
            $ui->assign('recibos_count', $recibos_count);

            $remitos_count = ORM::for_table('sys_remitos')
            ->where('userid', $id)
            ->count();
        ...
        if ($remitos_count == '') {
            ...
            $ui->assign('remitos_count', $remitos_count);
 ...
 ' . $extra_jq
    ...
    case 'list':
        ...
        if ($config['contact_set_view_mode'] == 'search') {
            ...
            $d = ORM::for_table('crm_accounts')
                ->where_raw('(`account` LIKE ? OR `company` LIKE ?)', ['%'.$name.'%', '%'.$name.'%'])               
...
_L[\'are_you_sure\'] = \'' .
 ...
 '
    ...
    case 'render-contacto':
        Event::trigger('contacts/render-contacto/');
        $cid = _post('cid');
        $d = ORM::for_table('crm_accounts')
        ->where('cid', $cid)
        ->find_many();
        echo '<label>Contacto</label><select id="coid" name="coid" class="form-control">';
        foreach ($d as $dcon) {
            echo '<option value="'.$dcon['id'].'">'.$dcon['account'].'</option>';
        }
        echo '</select>';
...
<table class="table table-bordered">
   ...
   <tbody>
    ...
    case 'render-contactos':
        Event::trigger('contacts/render-contactos/');
