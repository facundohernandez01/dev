<?php
_auth();
$ui->assign('_application_menu', 'remitos');
$ui->assign('_st', $_L['remitos']);
$ui->assign('_title', $_L['remitos'] . '- ' . $config['CompanyName']);
$action = $routes['1'];
$user = User::_info();
$ui->assign('user', $user);

Event::trigger('remitos');

switch ($action) {
    case 'add':
        //find all clients.

        Event::trigger('remitos/add/');

        $extra_fields = '';
        $extra_jq = '';

        Event::trigger('add_remitos');
        ...
        $ui->assign('_st', 'Nuevo recibo');
        $empresa = ORM::for_table('crm_accounts')
            ->select('id')
            ->select('cid')
            ->select('account')            
            ->order_by_desc('id')
            ->find_many();
        $ui->assign('empresa', $empresa);

        $t = ORM::for_table('sys_tax')->find_many();
        $ui->assign('t', $t);

        $ui->assign('_st', $_L['Add Invoice']);
        $tpt = ORM::for_table('sys_transportes')
            ->select('id')
            ->select('nombre_transporte')            
            ->order_by_desc('id')
            ->find_many();
        $ui->assign('tpt', $tpt);
        ...
        if ($config['i_driver'] == 'default') {
            $js_file = 'remito';
            $tpl_file = 'add-remito.tpl';
        } elseif ($config['i_driver'] == 'v2') {
            $js_file = 'remito_add_v2';
            $tpl_file = 'add_remito_v2.tpl';
        } else {
            $js_file = 'remito';
            $tpl_file = 'add-remito.tpl';
        }
 ...
 ' .
    ...
    case 'edit':
        ...
        $id = $routes['2'];
        $d = ORM::for_table('sys_remitos')->find_one($id);
        if ($d) {
            $currencies = Model::factory('Models_Currency')->find_array();
            $ui->assign('currencies', $currencies);

            $ui->assign('i', $d);
            $items = ORM::for_table('sys_remitositems')
                ->where('invoiceid', $id)
                ->order_by_asc('id')
                ->find_many();
            ...
            if ($config['i_driver'] == 'default') {
                $js_file = 'edit-remito-v2';
                $tpl_file = 'edit-remito.tpl';
            } elseif ($config['i_driver'] == 'v2') {
                $js_file = 'edit_remito_v2n';
                $tpl_file = 'edit_remito_v2.tpl';
            } else {
                $js_file = 'edit-remito-v2';
                $tpl_file = 'edit-remito.tpl';
            }
 ...
 '
    ...
    case 'view':
        ...
        if ($d) {
            ...
            $trsp = ORM::for_table('sys_transportes')
            ->where('id', $d['transporte_id'])
            ->order_by_desc('id')
            ->find_many();
            $ui->assign('trsp', $trsp);
    ...
    case 'add-post':
        Event::trigger('remitos/add-post/');
        ...
        if ($msg == '') {
            ...
            $vtoken = _raid(10);
            $ptoken = _raid(10);
            $d = ORM::for_table('sys_remitos')->create();
            $d->userid = $cid;
            $d->account = $u['account'];
            $d->date = $idate;
            $d->duedate = $dd;
            ...
            foreach ($description as $item) {
                ...
                //                }
                $ltotal = $samount * $sqty;
                $d = ORM::for_table('sys_remitositems')->create();
    ...
    case 'list':
        ...
        if (route(2) == 'filter') {
            ...
            $total_invoice = ORM::for_table('sys_remitos')->count();

            $ui->assign('total_invoice', $total_invoice);

            $f = ORM::for_table('sys_remitos');
            ...
            $paginator = Paginator::bootstrap('sys_remitos');
            $d = ORM::for_table('sys_remitos')
...
$(".cdelete").click(function (e) {
        ...
        bootbox.confirm("' .
           ...
           if(result){
               var _url = $("#_url").val();
               window.location.href = _url + "delete/remito/" + id;
           }
        });
    });
...
$(".cdelete").click(function (e) {
        ...
        bootbox.confirm("' .
           ...
           if(result){
               var _url = $("#_url").val();
               window.location.href = _url + "delete/remitos/" + id;
           }
        });
    });
     ...
     $(".cstop").click(function (e) {
        ...
        bootbox.confirm("Are you sure? This will prevent future invoice generation from this invoice.", function(result) {
           if(result){
               var _url = $("#_url").val();
               window.location.href = _url + "remitos/stop_recurring/" + id;
           }
        });
    });
 ...
 '
        ...
        $ui->display('list-recurring-remitos.tpl');
        break;

    case 'edit-post':
        Event::trigger('remitos/edit-post/');
        ...
        if ($msg == '') {
            ...
            // $vtoken = _raid(10);
            // $ptoken = _raid(10);
            $d = ORM::for_table('sys_remitos')->find_one($iid);
            if ($d) {
                $d->userid = $cid;
                $d->account = $u['account'];
                $d->date = $idate;
                ...
                $x = ORM::for_table('sys_remitositems')
                    ->where('invoiceid', $iid)
                    ->delete_many();
                foreach ($description as $item) {
                    $samount = $a[$i];
    ...
    case 'print':
        ...
        if ($d) {
            ...
            require 'application/lib/remitos/render.php';
        } else {
            r2(U . 'customers/list', 'e', $_L['Account_Not_Found']);
        }
    ...
    case 'pdf':
        ...
        if ($d) {
            ...
            $mpdf->SetTitle($config['CompanyName'] . ' Remito');
            ...
            Event::trigger('remitos/before_pdf_render/');

            ob_start();

            require 'application/lib/remitos/pdf-x2.php';
            ...
            if ($r_type == 'dl') {
                ...
                $mpdf->Output(
                    'application/storage/temp/Remito_' . $in . '.pdf',
                    'F'
                ); # D
            } else {
                $mpdf->Output(date('Y-m-d') . _raid(4) . '.pdf', 'I'); # D
    ...
    case 'markpaid':
        ...
        if ($d) {
            $d->status = 'Paid';
            $d->save();
            Event::trigger('remitos/markpaid/', $remito = $d);
    ...
    case 'markunpaid':
        Event::trigger('remitos/markunpaid/');
    ...
    case 'markcancelled':
        Event::trigger('remitos/markcancelled/');
    ...
    case 'markpartiallypaid':
        Event::trigger('remitos/markpartiallypaid/');
    ...
    case 'add-payment':
        Event::trigger('remitos/add-payment/');

        $sid = $routes['2'];
        $d = ORM::for_table('sys_remitos')->find_one($sid);
...
<input type="hidden" id="payer" name="payer" value="' .
    ...
    case 'mail_invoice_':
        Event::trigger('remitos/mail_invoice_/');
...
<div class="form-group">
    ...
    <div class="col-sm-10">
      <div class="checkbox c-checkbox">
                          <label>
                            <input type="checkbox" name="attach_pdf" id="attach_pdf" value="Yes" checked><span class="fa fa-check"></span>  <i class="fa fa-paperclip"></i> Remito_' .
                $in .
                '.pdf
                          </label>
                        </div>
...
<div class="modal-footer">
    ...
    case 'send_email':
        Event::trigger('remitos/send_email/');
        ...
        if ($attach_pdf == 'Yes') {
            Remito::pdf($iid, 'store');

            $attachment_path = 'storage/temp/Remito_' . $in . '.pdf';
            $attachment_file = 'Remito_' . $in . '.pdf';
        }
    ...
    case 'stop_recurring':
        Event::trigger('remitos/stop_recurring/');
    ...
    case 'add-payment-post':
        Event::trigger('remitos/add-payment-post/');
        ...
        if ($msg == '') {
            ...
            $i = ORM::for_table('sys_remitos')->find_one($iid);
            if ($i) {
                $pc = $i['credit'];
                $it = $i['total'];
                $dp = $it - $pc;
    ...
    case 'export_csv':
        ...
        $results = db_find_array('sys_remitos');
...
$(".cdelete").click(function (e) {
 ...
 '
    ...
    case 'clone':
        ...
        $new_id = Remito::cloneRemito($id);

        if ($new_id) {
            r2(U . 'remitos/edit/' . $new_id, 's', $_L['Cloned successfully']);
        }
