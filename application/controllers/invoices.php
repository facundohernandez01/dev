 ...
 '
    ...
    case 'view':
        ...
        if ($d) {
            ...
            $pedidos_remito_sub = ORM::for_table('sys_remitositems')    
            ->where('pedido_id', $id)
            ->find_many();
            $ui->assign('pedidos_remito_sub', $pedidos_remito_sub);
            $ui->assign('d', $d);

            $pedidos_remitos_sub_c = ORM::for_table('sys_remitositems')
            ->where('pedido_id', $id)
            ->sum('qty');
            $ui->assign('pedidos_remitos_sub_c', $pedidos_remitos_sub_c);

            $pedidos_recibos = ORM::for_table('sys_recibos')
            ->where('pedido_id', $id)
            ->find_many();
            $ui->assign('pedidos_recibos', $pedidos_recibos);

            $d2 = ORM::for_table('sys_recibos')
            ->where('pedido_id', $id)
            ->find_one();

            $pedidos_recibos_sub = ORM::for_table('sys_recibositems')            
            ->where('invoiceid', $d2['id'])
            ->find_many();
            $ui->assign('pedidos_recibos_sub', $pedidos_recibos_sub);
    ...
    case 'list':
        ...
        if (route(2) == 'filter') {
            ...
            if (route(3) != '') {
                ...
                if ($s_f == 'Finalizado') {
                    $f->where('status', 'Finalizado');
                } elseif ($s_f == 'Pendiente cobro') {
                    $f->where('status', 'Pendiente cobro');
                } elseif ($s_f == 'Pendiente entrega') {
                    $f->where('status', 'Pendiente entrega');
            ...
            $d = ORM::for_table('sys_invoices')
                ->where_not_equal('status', "Finalizado")
...
<div class="form-group">
    ...
    <div class="col-sm-10">
      <div class="checkbox c-checkbox">
                          <label>
                            <input type="checkbox" name="attach_pdf" id="attach_pdf" value="Yes" checked><span class="fa fa-check"></span>  <i class="fa fa-paperclip"></i> Pedido_' .
                $in .
                '.pdf
                          </label>
                        </div>
...
<div class="modal-footer">
    ...
    case 'add-payment-post':
        ...
        $iid = _post('iid');
        $q = ORM::for_table('sys_recibos')->find_one($iid);
...
$(".cdelete").click(function (e) {
 ...
 '
    ...
    case 'clone':
        ...
        case 'convert_arecibo':
            Event::trigger('invoices/convert_arecibo/');
            $iid = _post('iid');
            $q = ORM::for_table('sys_invoices')->find_one($iid);
            $q->recibo = 1;
            $q->save();
            ...
            if ($currency_find) {
                ...
                if ($msg == '') {
                    ...
                    $d->taxrate = $q['taxrate'];
                    $d->status = 'Paid';
                    $d->description = "Recibo asoc. pedido N° ".$iid;

   
                    //others
                    $d->invoicenum = '0001 00'.$iid;
                    $d->pedido_id = $iid;
                    $d->tax2 = '0.00';
                    $d->taxrate2 = '0.00';
                    $d->paymentmethod = '';
            ...
            case 'convert_arecibo_parcial':
                Event::trigger('invoices/convert_arecibo_parcial/');
                $iid = _post('iid');
                $q = ORM::for_table('sys_invoices')->find_one($iid);
                $q->recibo = 3;
                $q->save();
                ...
                if ($currency_find) {
                    ...
                    if ($msg == '') {
                        ...
                        $d->taxname = $q['taxname'];
                        $d->taxrate = $q['taxrate'];
                        $d->description = "Recibo generado desde pedido N° ".$iid;
            ...
            case 'convert_aremito':
                Event::trigger('invoices/convert_aremito/');
                $iid = _post('iid');
                $q = ORM::for_table('sys_invoices')->find_one($iid);
                ...
                elseIf ($remito && $recibo != 0)  {
                $estado = 'Finalizado';
                }
                elseIf ($remito == 0 && $recibo != 1)  {
                $estado = 'Pendiente entrega';
                }
                elseIf ($remito == 1 && $recibo != 0)  {
            ...
            if ($currency_find) {
                ...
                $currency_rate = 1.0;
                    ...
                    if ($msg == '') {
                        ...
                        $d->taxrate = $q['taxrate'];
                        $d->status = $estado;
                        $d->description = "Remito generado desde pedido N° ".$iid;
                        $d->pedido_id = $iid;
                        //others
                        $d->invoicenum = '0001 000';
                        $d->tax2 = '0.00';
                        $d->taxrate2 = '0.00';
                        $d->paymentmethod = '';
                        //
                ...
                case 'convert_aremito_parcial':
                    Event::trigger('invoices/convert_aremito_parcial/');
                    $iid = _post('iid');
                    $q = ORM::for_table('sys_invoices')->find_one($iid);
                    $q->remito = 3;
                    $q->save();
