...
$ui->assign('_st', 'Recibos');
$ui->assign('_title', $_L['Sales'] . '- ' . $config['CompanyName']);
$action = $routes['1'];
$user = User::_info();
$ui->assign('user', $user);

Event::trigger('recibos');

switch ($action) {
    case 'add':
        //find all clients.

        Event::trigger('recibos/add/');

        $extra_fields = '';
        $extra_jq = '';

        Event::trigger('add_recibos');
        ...
        if ($config['i_driver'] == 'default') {
            $js_file = 'recibo';
            $tpl_file = 'add-recibo.tpl';
        } elseif ($config['i_driver'] == 'v2') {
            $js_file = 'recibo_add_v2';
            $tpl_file = 'add_recibo_v2.tpl';
        } else {
            $js_file = 'recibo';
            $tpl_file = 'add-recibo.tpl';
        }
 ...
 ' .
    ...
    case 'edit':
        ...
        if ($d) {
            ...
            if ($config['i_driver'] == 'default') {
                $js_file = 'edit-recibo-v2';
                $tpl_file = 'edit-recibo.tpl';
            } elseif ($config['i_driver'] == 'v2') {
                $js_file = 'edit_recibo_v2n';
                $tpl_file = 'edit_recibo_v2.tpl';
            } else {
                $js_file = 'edit-recibo-v2';
                $tpl_file = 'edit-recibo.tpl';
            }
 ...
 '
    ...
    case 'add-post':
        Event::trigger('recibos/add-post/');
        ...
        if ($msg == '') {
            ...
            $vtoken = _raid(10);
            $ptoken = _raid(10);
            $d = ORM::for_table('sys_recibos')->create();
            $d->userid = $coid;
            $d->account = $u['account'];
            $d->date = $idate;
            $d->duedate = $dd;
            ...
            foreach ($description as $item) {
                ...
                //                }
                $ltotal = $samount * $sqty;
                $d = ORM::for_table('sys_recibositems')->create();
    ...
    case 'list':
        ...
        if (route(2) == 'filter') {
            ...
            $total_invoice = ORM::for_table('sys_recibos')->count();

            $ui->assign('total_invoice', $total_invoice);

            $f = ORM::for_table('sys_recibos');
            ...
            $paginator = Paginator::bootstrap('sys_recibos');
            $d = ORM::for_table('sys_recibos')
...
$(".cdelete").click(function (e) {
        ...
        bootbox.confirm("' .
           ...
           if(result){
               var _url = $("#_url").val();
               window.location.href = _url + "delete/recibo/" + id;
           }
        });
    });
...
$(".cdelete").click(function (e) {
        ...
        bootbox.confirm("' .
           ...
           if(result){
               var _url = $("#_url").val();
               window.location.href = _url + "delete/recibos/" + id;
           }
        });
    });
     ...
     $(".cstop").click(function (e) {
        ...
        bootbox.confirm("Are you sure? This will prevent future invoice generation from this invoice.", function(result) {
           if(result){
               var _url = $("#_url").val();
               window.location.href = _url + "recibos/stop_recurring/" + id;
           }
        });
    });
 ...
 '
        ...
        $ui->display('list-recurring-recibos.tpl');
        break;
        case 'add_company_post_from':
            $data = ib_posted_data();
            ...
            case 'edit-post':
                Event::trigger('recibos/edit-post/');
                ...
                if ($msg == '') {
                    ...
                    // $vtoken = _raid(10);
                    // $ptoken = _raid(10);
                    $d = ORM::for_table('sys_recibos')->find_one($iid);
                    if ($d) {
                        $d->userid = $cid;
                        $d->account = $u['account'];
                        $d->date = $idate;
                        ...
                        $x = ORM::for_table('sys_recibositems')
                            ->where('invoiceid', $iid)
                            ->delete_many();
                        foreach ($description as $item) {
                            $samount = $a[$i];
    ...
    case 'pdf':
        ...
        if ($d) {
            ...
            $mpdf->SetTitle($config['CompanyName'] . ' Recibo');
            ...
            Event::trigger('recibos/before_pdf_render/');

            ob_start();

            require 'application/lib/recibos/pdf-x2.php';
            ...
            if ($r_type == 'dl') {
                ...
                $mpdf->Output(
                    'application/storage/temp/Recibo_' . $in . '.pdf',
                    'F'
                ); # D
            } else {
                $mpdf->Output(date('Y-m-d') . _raid(4) . '.pdf', 'I'); # D
    ...
    case 'markpaid':
        ...
        if ($d) {
            $d->status = 'Paid';
            $d->save();
            Event::trigger('recibos/markpaid/', $recibo = $d);
    ...
    case 'markunpaid':
        Event::trigger('recibos/markunpaid/');
    ...
    case 'markcancelled':
        Event::trigger('recibos/markcancelled/');
    ...
    case 'markpartiallypaid':
        Event::trigger('recibos/markpartiallypaid/');
        ...
        if ($d) {
            ...
            $d->save();
            _msglog('s', 'Marcado como Pago parcial');
        } else {
            _msglog('e', 'not found');
        }
        break;
    ...
    case 'add-payment':
        Event::trigger('recibos/add-payment/');

        $sid = $routes['2'];
        $d = ORM::for_table('sys_recibos')->find_one($sid);
...
<div class="form-group">
    <label for="amount" class="col-sm-2 control-label">' .
                ...
                '</label>
    <div class="col-sm-10">
      <input type="text" id="amount" name="amount" class="form-control amount"   data-a-sign="" data-a-dec="' .
                $config['dec_point'] .
                '" data-a-sep="' .
                $config['thousands_sep'] .
                '"
...
<input type="hidden" id="payer" name="payer" value="' .
    ...
    case 'mail_invoice_':
        Event::trigger('recibos/mail_invoice_/');
...
<div class="form-group">
    ...
    <div class="col-sm-10">
      <div class="checkbox c-checkbox">
                          <label>
                            <input type="checkbox" name="attach_pdf" id="attach_pdf" value="Yes" checked><span class="fa fa-check"></span>  <i class="fa fa-paperclip"></i> Recibo_' .
                $in .
                '.pdf
                          </label>
                        </div>
...
<div class="modal-footer">
    ...
    case 'send_email':
        Event::trigger('recibos/send_email/');
        ...
        if ($attach_pdf == 'Yes') {
            Recibo::pdf($iid, 'store');

            $attachment_path = 'storage/temp/Recibo_' . $in . '.pdf';
            $attachment_file = 'Recibo_' . $in . '.pdf';
        }
    ...
    case 'stop_recurring':
        Event::trigger('recibos/stop_recurring/');
    ...
    case 'add-payment-post':
        Event::trigger('recibos/add-payment-post/');
        ...
        if ($msg == '') {
            ...
            $i = ORM::for_table('sys_recibos')->find_one($iid);
            if ($i) {
                $pc = $i['credit'];
                $it = $i['total'];
                $dp = $it - $pc;
    ...
    case 'export_csv':
        ...
        $results = db_find_array('sys_recibos');
...
$(".cdelete").click(function (e) {
 ...
 '
    ...
    case 'clone':
        ...
        $new_id = Recibo::cloneRecibo($id);

        if ($new_id) {
            r2(U . 'recibos/edit/' . $new_id, 's', $_L['Cloned successfully']);
        }
