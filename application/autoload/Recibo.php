...
class Recibo
{
    ...
    public static function pdf($id, $r_type = '', $token = '')
    {
        ...
        if ($d) {
            ...
            $pdf_tpl = 'application/lib/recibos/pdf-x2.php';

            Event::trigger('recibos/before_pdf_render/', [$id]);
            ...
            if ($r_type == 'dl') {
                ...
                $mpdf->Output('storage/temp/Rercibo_' . $in . '.pdf', 'F'); # D
            } else {
                $mpdf->Output(date('Y-m-d') . _raid(4) . '.pdf', 'I'); # D
            }
        }
    ...
    public static function forSingleItem(
        ...
        $msg = Recibo::gen_email($invoiceid, 'created');
        ...
        if ($email != '') {
            Recibo::pdf($invoiceid, 'store');

            $attachment_path =
                'application/storage/temp/Recibo_' . $invoiceid . '.pdf';
            $attachment_file = 'Recibo_' . $invoiceid . '.pdf';
    ...
    public static function cloneRecibo($id)
    {
        $inv = ORM::for_table('sys_recibos')->find_one($id);
        ...
        if ($inv) {
            ...
            foreach ($items as $item) {
                $t = ORM::for_table('sys_recibositems')->create();
