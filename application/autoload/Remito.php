...
class Remito
{
    ...
    public static function pdf($id, $r_type = '', $token = '')
    {
        ...
        if ($d) {
            if ($token != '') {
                ...
                $vtoken = $d->vtoken;
                if ($token != $vtoken) {
                    echo 'Sorry Token does not matchsssss!';
                    exit();
                }
            }
            ...
            $pdf_tpl = 'application/lib/remitos/pdf-x2.php';

            Event::trigger('remitos/before_pdf_render/', [$id]);
            ...
            if ($r_type == 'dl') {
                ...
                $mpdf->Output('storage/temp/Remito_' . $in . '.pdf', 'F'); # D
            } else {
                $mpdf->Output(date('Y-m-d') . _raid(4) . '.pdf', 'I'); # D
            }
        }
    ...
    public static function forSingleItem(
        ...
        $msg = Remito::gen_email($invoiceid, 'created');
        ...
        if ($email != '') {
            Remito::pdf($invoiceid, 'store');

            $attachment_path =
                'application/storage/temp/Remito_' . $invoiceid . '.pdf';
            $attachment_file = 'Remito_' . $invoiceid . '.pdf';
            Notify_Email::_send(Rercibo
                $name,
                $email,
                $subj,
                $message_o,
    ...
    public static function cloneRemito($id)
    {
        $inv = ORM::for_table('sys_remitos')->find_one($id);
        ...
        if ($inv) {
            ...
            foreach ($items as $item) {
                $t = ORM::for_table('sys_remitositems')->create();
